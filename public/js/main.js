...
//ethic values
var values = ["privacy","participation","accountability","autonomy","responsibility","dignity","transparency","non-discrimination","inclusion-equality","interoperability","data-protection","safety-security","wellbeing"]

var valuesTxt = ["Privacy","Participation","Accountability","Autonomy","Responsibility","Dignity","Transparency","Non-Discrimination","Inclusion & Equality","Interoperability","Data Protection","Safety & Security","Wellbeing"]
  ...
  if (state == 0) {
    if (new_group.isActive != true) {
      ...
      for (var i = 0; i < stack.length; i++) {
        ...
        stack[i].animate('shadow.blur', '15', {
          duration: 250,
          onChange: canvas.renderAll.bind(canvas),
          onComplete: function() {
            //callback code goes here
...
function highligt_the_stack(the_group) {
  if (state == 0) {
    if (the_group != old_highlight_group) {
	    ...
	    for (var e = the_group.getObjects().length - 1; e >= 0; e--) {
	      if (the_group.getObjects()[e].id == "title") {
	        the_group.getObjects()[e].set({
	          fontSize: 20
	          // opacity: 1
	        })
	      }
	    }      
      the_group.animate('shadow.blur', '15', {
        duration: 100,
        onChange: canvas.renderAll.bind(canvas),
        onComplete: function() {
          //callback code goes here
      ...
      if (old_highlight_group != undefined && !old_highlight_group.isActive) {
	    ...
	    for (var e = old_highlight_group.getObjects().length - 1; e >= 0; e--) {
	      if (old_highlight_group.getObjects()[e].id == "title") {
	        old_highlight_group.getObjects()[e].set({
	          fontSize: 18,
	          opacity:.75
	        })
	      }
...
function expose_the_stack(){
  ...
  document.getElementById("expose").classList.remove("pulseSlow");
  ...
  document.getElementById('modal-text').innerHTML = "You've mapped your product in relation to your ethical values. In this third stage, we challenge how you understand those values.<br><br>Why? <br><br>The values that are important to you ripple through the product's functionality, so if any are shaky, it will shake the whole stack. We question each value and your answers will determine whether your product has a rock-solid or shaky ethical foundation. We will identify the answers that have the most risk from an ethical and social point of view. Risk can include harm to your users, to your own values and therefore your company's foundation and team, or to your overall public reputation and relations.<br><br>These areas do not cover all of the possible issues you may have. If you want to go through all of the issues, use VIRT-EU's long-form privacy, ethical and social impact assessment questionnaire: ";
  var newlink = document.createElement('a');
  	var linkname = document.createTextNode("VIRT-EU PESIA");
  	newlink.appendChild(linkname);
  	newlink.title = "VIRT-EU PESIA"
    newlink.href = 'https://www.virteuproject.eu/servicepackage/pesia/';
...
function goToNextStep() {
  ...
  if (state == 1) {
    ...
    document.getElementById("expose").classList.add("pulseSlow");
    ...
    try {
      if (options.subTargets[0].isValue != true) {
        ...
        if (problem_values.indexOf(line.from) != -1) {
        	...
        	console.log(problem_values.indexOf(line.from))
        }
...
function showGroup(group) {
	document.getElementById("the-countdown").innerHTML = "Questions remaining: ";
	document.getElementById("the-countnum").innerHTML = amount_answered-1;
